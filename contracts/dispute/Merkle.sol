...
contract Merkle {
    ...
    function process(bytes32 leaf, bytes[] memory inst) internal pure returns (bytes32) {
	// first is the leaf
	for (uint i = 0; i+1 < inst.length; i += 2) {
	    leaf = keccak256(abi.encodePacked(inst[i], leaf, inst[i+1]));
	}
	return leaf;
    }
   
    function slice(bytes memory arr, uint i, uint n) internal pure returns (bytes memory) {
	bytes memory res = new bytes(n);
	for (uint j = 0; j < n; j++) res[j] = arr[i+j];
	return res;
    }
   
    function slice2(bytes memory arr, uint i1, uint i2) internal pure returns (bytes memory) {
    ...
    function process2(bytes32 leaf, uint[] memory ctrl, bytes memory inst) public pure returns (bytes32) {
	// first is the leaf
	for (uint i = 1; i+2 < ctrl.length; i += 2) {
	    leaf = keccak256(abi.encodePacked(slice2(inst, ctrl[i], ctrl[i+1]), leaf, slice2(inst, ctrl[i+1], ctrl[i+2])));
	}
	return leaf;
    }
    ...
    function dataMerkle(uint[] memory ctrl, uint idx, uint level) internal pure returns (bytes32) {
	if (level == 0) {
	    if (idx < ctrl.length) {
		// get the element
		bytes32 elem = bytes32(ctrl[idx]);
		// return keccak256(abi.encodePacked(bytes16(elem), uint128(elem)));
		return keccak256(abi.encodePacked(elem));
	    }
	    else return keccak256(abi.encodePacked(bytes16(0), bytes16(0)));
	}
	else return keccak256(abi.encodePacked(dataMerkle(ctrl, idx, level-1), dataMerkle(ctrl, idx+(2**(level-1)), level-1)));
    }

    function test(bytes memory dta) public pure returns (bytes32) {
	return keccak256(abi.encodePacked(dta));
    }

    function test2(bytes memory dta, uint n) public pure {
	slice2(dta, 0, n);
    }

    function test3() public pure returns (bytes32) {
	return keccak256(abi.encodePacked(bytes2(0x0)));
    }
	...
	bytes32 id = keccak256(abi.encodePacked(state, solver));
	...
	// bytes32 leaf = keccak256(abi.encodePacked(bytes16(t.leaf), uint128(t.leaf)));
	bytes32 leaf = keccak256(abi.encodePacked(t.leaf));
	// bytes32 root = keccak256(bytes16(t.valid_root), uint128(t.valid_root));
	return size == 32 && state == keccak256(abi.encodePacked(keccak256(abi.encodePacked(leaf, zero)), keccak256(abi.encodePacked(zero, zero))));
    }

    function submitProof(bytes32 id, bytes32 leaf, uint[] memory ctrl, bytes memory inst, uint sz) public {
	Task storage t = tasks[id];
